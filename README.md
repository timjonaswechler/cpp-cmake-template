...
In this `CMakePresets.json` example, a preset named `default` is defined. It uses the Ninja generator and sets the binary directory relative to the source directory. The `cacheVariables` set important project variables, like the build type and C++ standard. For more information on CMakePresets, refer to the [official CMake documentation](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html).
...
To start working with the CMake template, you first need to clone the repository to your local machine. Cloning creates a local copy of the project and its entire version history.
